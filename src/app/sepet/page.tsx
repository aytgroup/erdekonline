...
export default function SepetPage() {
  ...
  function siparisVer() {
    try {
      ...
      const yeniSiparis = {
        ...
        toplam: items.reduce((acc, i) => acc + i.fiyat * i.adet, 0) + (toplam >= 300 ? 0 : 25),
  ...
  const teslimat = items.length > 0 ? (toplam >= 300 ? 0 : 25) : 0;
  const genel = toplam + teslimat;
...
<div className="lg:col-span-1">
              <div className="bg-white rounded-2xl border border-gray-100 p-6 sticky top-24">
                ...
                <div className="flex flex-col gap-2 text-sm text-gray-600 mb-4">
                  ...
                  <div className="flex justify-between">
                    <span>Teslimat</span>
                    <span className={teslimat === 0 ? "text-green-600 font-semibold" : "font-semibold text-gray-800"}>
                      {teslimat === 0 ? "Ücretsiz 🎉" : `${teslimat} ₺`}
                    </span>
                  </div>
                  {teslimat > 0 && <p className="text-xs text-gray-400">300 ₺ üzeri siparişlerde ücretsiz teslimat!</p>}
                </div>
